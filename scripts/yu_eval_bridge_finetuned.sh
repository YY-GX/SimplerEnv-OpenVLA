...
ckpt_path="/mnt/bum/yufang/projects/openvla/runs/1.0.4/openvla-7b+droid+b8+lr-0.0001+lora-r32+dropout-0.0--image_aug--1000_chkpt"  # carrot
ckpt_path="/mnt/bum/yufang/projects/openvla/runs/1.0.5/openvla-7b+droid+b8+lr-0.0001+lora-r32+dropout-0.0--image_aug--1000_chkpt"  # spoon
...
for seed in "${seeds[@]}"; do
  for env_name in "${set_1[@]}"; do
    if [[ "$env_name" == "PutEggplantInBasketScene-v0" ]]; then
      ...
      CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference.py --my_folder "evaluate_on_finetuned_openvla_spoon" --policy-model ${policy_model} --ckpt-path ${ckpt_path} \
